...
export default function CalendarConsentButton() {
    ...
    // 2. Funci�n para manejar la desconexi�n
    const handleDisconnect = async () => {
        if (!uid) return;
        if (!confirm("�Est�s seguro de que quieres desvincular tu Google Calendar?")) {
            return;
        }
    ...
    return (
        <div className="flex items-center gap-2">
            {connected ? (
                // 3. Bot�n "Conectado" con efecto hover
                <button
                ...
                // Bot�n "Desconectado" (sin cambios)
                <a
