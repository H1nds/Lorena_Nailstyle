...
export default function Dashboard({ user, onLogout }: { user: string; onLogout: () => void }) {
    ...
    const handleSave = async (s: Sale) => {
        try {
            if (editing) {
                // updateSale puede ser async cuando usamos Firestore
                await updateSale(s);
                setEditing(null);
            } else {
                await addSale(s);
        ...
        <div className="min-h-screen p-6 text-left" style={{ backgroundColor: "var(--bg-main)", color: "var(--text-main)", }}>
            <header className="flex items-center justify-between mb-6">
                <div>
                    ...
                    <p className="text-sm text-gray-600">Usuario: {user}</p>
                </div>
                ...
                <div className="flex items-center gap-3">
                    <button
                        ...
                        style={{
                            backgroundColor: "#FFF4D9", // amarillo pastel plano
                            border: "1px solid rgba(250,204,21,0.12)" // sutil borde para separación
                        }}
                    >
                    ...
                    <button
                        ...
                        className="btn-icon bg-green-100 text-green-800 p-2 rounded-full"
                    ...
                    <button
                        ...
                        className="btn-icon bg-blue-100 text-blue-800 p-2 rounded-full"
                    ...
                    <button
                        onClick={() => onLogout()}
                        className="btn-icon bg-pink-100 text-pink-800 p-2 rounded-full"
                        title="Cerrar sesión"
